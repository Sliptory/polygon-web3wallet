...
async function signMessage() {
  ...
  let message = `Hello Comrads!!! GUID = ${id}`;
  try {
    ...
    if(isMob())
      ...
      webResponse("Signature complete.<br><br>Copy to clipboard then continue to App", signature, message);
  } catch (error) {
...
function webResponse(text, signature, message) {
...
function mobResponse(text, signature, message) {
  ...
  if (signature && message) {
    ...
    responseButton.onclick = () => copyToClipboard(JSON.stringify(json));
  }
}
